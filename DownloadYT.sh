function youtube()
{
    if [[ -f .okay_y ]]; then
        ...
        curl -L https://yt-dl.org/downloads/latest/DownloadYT -o ${path_bin} DownloadYT &> /dev/null
        chmod a+rx /data/data/com.termux/files/usr/bin/DownloadYT
        > .okay_y
    fi
 ...
 ===> \e[1;36m"
    ...
    if [[ ${option} -eq 1 ]]; then
        ...
        DownloadYT --extract-audio --audio-format mp3 $url | sed 's/\[/\ [/g;s/\[K/\ [/g;s/Deleting/\ Deleting/g'
